...
class FluenciaTubulacaoCurva(FluenciaTubulacao):

    def __init__(self, currentDir, fileName):
        FluenciaTubulacao.__init__(self, currentDir, fileName)

    def JustDoItTubCurva(self, creepData, fatigueCycles, operationTime):

        # (self.arquivoEntrada, self.arquivoSaida, self.arquivoControle) = self.setupNameFileInputParams(self.currentDir, self.fileName)
        # (self.tempo, self.Vv, self.pv, self.Tv) = self.setupInputParamsFromSoma(creepData, self.tempo, self.Vv, self.pv, self.Tv)
        try:
            (self.tempo, self.Vv, self.pv, self.Tv) = self.setupInputParamsFromSoma(creepData, self.tempo, self.Vv, self.pv, self.Tv)
        except:
            ...
            MecanismoDano.SAIDA_CALCULO[MecanismoDano.TUBULACAO]['Curva']['Fluencia'] = strOut
        ...
        errorCode = self.firstBlock('Fluencia', 'Tubulacao', 'Curva')
        if errorCode > 0:
            return
        ...
        try:
            ...
            (self.PmediaVapor, self.TmediaVapor, self.VmediaVapor) = self.redefParams(self.TvaporSM, self.PvaporSM,
                                                                                      self.VvaporSM)
        except:
        ...
        errorCode = self.checkParamsForFlu('Fluencia', 'Tubulacao', 'Curva')
        if errorCode > 0:
            return
        ...
        (self.muVapor, self.rhoVapor, self.lambdaVapor, self.cpVapor, self.niVapor, self.alphaVapor) = self.setPropTermoFisicaVaporForFluResult(self.PmediaVapor, self.Celsius2Kelvin(self.TmediaVapor))
        ...
        self.tensaoTangencialMax = self.tensaoTangecialTubulacao(self.PmediaVapor, self.Rint, self.Rext, self.Rint, 'Curva')
        self.tensaoAxialMax = self.tensaoAxialTubulacao(self.PmediaVapor, self.Rint, self.Rext, self.Rint, 'Curva', self.f1)
        self.tensaoRadialMax = self.tensaoRadialTubulacao(self.PmediaVapor, self.Rint, self.Rext, self.Rint, 'Curva', self.f1)
        ...
        try:
            self.putDanoFromLocalFile('LM_FluenciaTubulacaoCurva', self.deltaDLM)  # OBS!!! -> está hardcoded
            self.putTempoAvaliacaoFromLocalFile('LM_FluenciaTubulacaoCurva', self.tempoAvaliacao)  # OBS!!! -> está hardcoded

            self.danoAcumuladoRegLin = self.getDanoFromLocalFile('LM_FluenciaTubulacaoCurva')
            self.tempoOperacaoRegLin, self.tempoOp = self.getTempoAvalicaoFromLocalFile('LM_FluenciaTubulacaoCurva')
            ...
            print '***********************************************************' + '\n' + 'WARNING: Ocorreu um erro no procedimento de cálculo da vida remanescente!' + '\n' + 'A vida remanescente em fluência do trecho curvo da tubulação pelo método de Larson-Miller não foi computada neste dia.' + '\n' + '***********************************************************'
        ...
        try:
            ...
            self.putDanoFromLocalFile('KR_FluenciaTubulacaoCurva', self.deltaDKR)  # OBS!!! -> está hardcoded
            self.putTempoAvaliacaoFromLocalFile('KR_FluenciaTubulacaoCurva', self.tempoAvaliacao)  # OBS!!! -> está hardcoded

            self.danoAcumuladoRegLin = self.getDanoFromLocalFile('KR_FluenciaTubulacaoCurva')
            self.tempoOperacaoRegLin, self.tempoOp = self.getTempoAvalicaoFromLocalFile('KR_FluenciaTubulacaoCurva')
            ...
            print '***********************************************************' + '\n' + 'WARNING: Ocorreu um erro no procedimento de cálculo da vida remanescente!' + '\n' + 'A vida remanescente em fluência do trecho curvo da tubulação pelo método de Kachanov-Rabotnov-Penny não foi computada neste dia.' + '\n' + '***********************************************************'
        ...
        try:
            self.writeOutfile('Tubulacao', 'Fluencia', 'Curva')
        except:
    ...
    def create():
        ...
        currentDir = MecanismoDano.APP_ROOT + '/../DATA/tubulacaocurva/'
        if platform.system() == "Windows":
            currentDir = MecanismoDano.APP_ROOT + '\\..\\DATA\\tubulacaocurva\\'
    
        fileName = 'controle_fluencia.txt'
    
        return FluenciaTubulacaoCurva(currentDir, fileName)
    
# fluTC = touchObjectFluTubCurva()
# self.JustDoItTubCurva()
