...
class FluenciaValvulaCurva(FluenciaValvula):
    def __init__(self, currentDir, fileName):
        FluenciaValvula.__init__(self, currentDir, fileName)

    def JustDoItValvCurva(self, creepData, fatigueCycles, operationTime):
        ...
        try:
            ...
            MecanismoDano.SAIDA_CALCULO[MecanismoDano.VALVULA]['Curva']['Fluencia'] = strOut
            return

        # (self.tempoOperacaoBase, self.tempo, self.Vv, self.pv, self.Tv, self.Tmint, self.Tmext) = self.setupInputParams(self.arquivoEntrada, self.tempo, self.Vv, self.pv, self.Tv, self.Tmint, self.Tmext)
        ...
        errorCode = self.firstBlock('Fluencia', 'Valvula', 'Curva')
        if errorCode > 0:
            return
        ...
        errorCode = self.checkParamsForFlu('Fluencia', 'Valvula', 'Curva')
        if errorCode > 0:
            return
        ...
        self.tensaoTangencialMax = self.tensaoTangecialTubulacao(self.PmediaVapor, self.Rint, self.Rext,
                                                                   self.Rint, 'Curva')
        self.tensaoAxialMax = self.tensaoAxialTubulacao(self.PmediaVapor, self.Rint, self.Rext, self.Rint,
                                                          'Curva', self.f1)
        ...
        self.tRKR = self.geTtRKR(self.tensaoEquivalenteMax, self.Twallmedia)  # método de Kachanov-Rabotnov
        ...
        self.deltaDLM = self.deltaD(self.tRLM, self.tempoAvaliacao) # delta -> acredito que represente "acréscimo"
        self.deltaDKR = self.deltaD(self.tRKR, self.tempoAvaliacao) # deltaD -> é o acréscimo de dano em porcentagem

        ### Regressão linear para estimar a vida remanescente em fluência por Larson-Miller ###

        try:
            self.putDanoFromLocalFile('LM_FluenciaValvulaCurva', self.deltaDLM)  # OBS!!! -> está hardcoded
            self.putTempoAvaliacaoFromLocalFile('LM_FluenciaValvulaCurva', self.tempoAvaliacao)  # OBS!!! -> está hardcoded

            self.danoAcumuladoRegLin = self.getDanoFromLocalFile('LM_FluenciaValvulaCurva')
            self.tempoOperacaoRegLin, self.tempoOp = self.getTempoAvalicaoFromLocalFile('LM_FluenciaValvulaCurva')
            ...
            self.covariance_RegLin = self.covariance(self.tempoOperacaoRegLin, self.mean_x_RegLin, self.danoAcumuladoRegLin, self.mean_y_RegLin)

            self.coefficients_RegLin = self.coefficients(self.mean_x_RegLin, self.mean_y_RegLin, self.variance_x_RegLin, self.covariance_RegLin)
            ...
            print '***********************************************************' + '\n' + 'WARNING: Ocorreu um erro no procedimento de cálculo da vida remanescente!' + '\n' + 'A vida remanescente em fluência do trecho curvo da válvula pelo método de Larson-Miller não foi computada neste dia.' + '\n' + '***********************************************************'
        ...
        try:
            ...
            self.putDanoFromLocalFile('KR_FluenciaValvulaCurva', self.deltaDKR)  # OBS!!! -> está hardcoded
            self.putTempoAvaliacaoFromLocalFile('KR_FluenciaValvulaCurva', self.tempoAvaliacao)  # OBS!!! -> está hardcoded

            self.danoAcumuladoRegLin = self.getDanoFromLocalFile('KR_FluenciaValvulaCurva')
            self.tempoOperacaoRegLin, self.tempoOp = self.getTempoAvalicaoFromLocalFile('KR_FluenciaValvulaCurva')
            ...
            print '***********************************************************' + '\n' + 'WARNING: Ocorreu um erro no procedimento de cálculo da vida remanescente!' + '\n' + 'A vida remanescente em fluência do trecho curvo da válvula pelo método de Kachanov-Rabotnov-Penny não foi computada neste dia.' + '\n' + '***********************************************************'
            self.vidaKR_RegLin = 0
        ...
        try:
            self.writeOutfile('Valvula', 'Fluencia', 'Curva')
        except:
    ...
    def create():
        ...
        currentDir = MecanismoDano.APP_ROOT + '/../DATA/valvulacurva/'
        if platform.system() == "Windows":
            currentDir = MecanismoDano.APP_ROOT + '\\..\\DATA\\valvulacurva\\'
    
        fileName = 'controle_fluencia.txt'
    
        return FluenciaValvulaCurva(currentDir, fileName)
...
#     return FluenciaValvulaCurva(currentDir, fileName)
# #         #
# fluVC = touchObjectFadRotor()
# fluVC.JustDoItValvCurva([1], 324, 10)


# fluVC = touchObjectFluValvCurva()
# self.JustDoItValvCurva()
